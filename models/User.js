const mongoose = require('mongoose');

const UserSchema = new mongoose.Schema({
  name: { type: String, required: [true, "O nome é obrigatório."] },
  username: { type: String, required: [true, "Digite um nome de usuário"] },
  email: { type: String, required: [true, "Digite um e-mail"], unique: true },
  otp: { type: String, required: false, default: "none" },
  fcm: { type: String, required: false, default: "none" },
  password: { type: String, default: true },
  verification: { type: Boolean, default: false },
  phone: { type: String, default: "71991509770" },
  phoneVerification: { type: Boolean, default: false },
  address: {
    type: mongoose.Schema.Types.ObjectId,
    ref: "Address",
    required: false
  },
  userType: { type: String, required: true, default: "Client", enum: ['Client', 'Admin', 'Vendor', 'Driver'] },
  profile: { type: String, default: "https://d326fntlu7tb1e.cloudfront.net/uploads/bdec9d7d-0544-4fc4-823d-3b898f6dbbbf-vinci_03.jpeg" },
}, { timestamps: true });

module.exports = mongoose.model('User', UserSchema);