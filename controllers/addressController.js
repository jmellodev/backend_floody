...
module.exports = {
  addAddress: async (req, res) => {
    ...
    try {
      ...
      res.status(201).json({ status: true, message: "Endereço adicionado." })
    } catch (error) {
  ...
  deleteAddress: async (req, res) => {
    try {
      ...
      res.status(200).json({ status: true, message: "Endereço excluído." })
    } catch (error) {
      res.status(500).json({ status: false, message: error.message })
    }
  },
  ...
  setAddressDefault: async (req, res) => {
    ...
    try {
      ...
      if (updatedAddress) {
        ...
        res.status(200).json({ status: true, message: "Endereço adicionado como padrão." })
      } else {
        res.status(400).json({ status: false, message: "endereço não encontrado" })
      }
    } catch (error) {
      res.status(500).json({ status: false, message: error.message })
    }
